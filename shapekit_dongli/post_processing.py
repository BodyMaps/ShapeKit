...
supported_class_map = getattr(class_maps, "supported_class_map")
...
class OrganRectifier:
    ...
    def remove_out_of_bounds_components(
        ...
        if removed_count > 0:
            logger.debug(
                f"{removed_count} out of {num_comps} {supported_class_map[target_label]} components outside reference organ bounds are removed."
            )
            labeled_comps, num_comps = ndi.label(seg_masks[target_label])
    ...
    def reassign_components(
        ...
        if reassigned_labels:
            logger.debug(
                f"{len(reassigned_labels)} out of {num_comps} {supported_class_map[target_label]} components are reassigned to other organs."
            )
    ...
    def apply_bladder_or_prostate_postprocessing(
        ...
        ref_width = ref_coords[:, 0].ptp() if ref_coords.size > 0 else np.inf
        ...
        if removed_count > 0:
            logger.debug(
                ...
                f"{supported_class_map[target_label]} components outside reference bounds are removed."
            )
    ...
    def apply_lung_postprocessing(
        ...
        if primary_lung_idx == -1:
            logger.debug(
                f"No valid {supported_class_map[target_label]} component found above abdominal organs."
            )
        ...
        if reassigned_count > 0:
            logger.debug(
                f"{reassigned_count} of {num_comps} {supported_class_map[target_label]} components reassigned to {supported_class_map[fallback_label]}."
            )
    ...
    def apply_pancreatic_duct_post_processing(
        ...
        if pancreas_coords.size != 0:
            ...
            logger.debug(
                f"No pancreas found for {supported_class_map[target_label]} post-processing."
            )
            return seg_masks
        ...
        if removed_count > 0:
            logger.debug(
                f"{removed_count} out of {num_comps} {supported_class_map[target_label]} components outside pancreas bounds are removed."
            )
    ...
    def _get_reference_bounds(
        ...
        if target_label in [1, 2]:  # adrenal glands
            ...
            logger.debug(
                f"No pre-defined reference bounds for {supported_class_map[target_label]}"
            )
        ...
        if target_label in [3, 22]:  # aorta, postcava
            if 12 in seg_masks and 13 in seg_masks:  # kidney
                ...
                if left_kidney_coords.size != 0:
                    left_kidney_height = left_kidney_coords[:, 2].ptp()
                else:
                    left_kidney_height = -np.inf

                if right_kidney_coords.size != 0:
                    right_kidney_height = right_kidney_coords[:, 2].ptp()
                else:
                ...
                for i in range(1, num_comps + 1):
                    comp_mask = labeled_comps == i
                    comp_coords = np.argwhere(comp_mask)
                    comp_height = comp_coords[:, 2].ptp()
                    if comp_height >= max_kidney_height:
                        keep_fragmented_labels.pop(
                            supported_class_map[target_label], None
                        )
                        break
        return seg_masks
    ...
    def _validate_and_label_components(
        ...
        if target_label not in seg_masks:
            logger.debug(
                f"Label {target_label} not found. Skip processing."
            )
            return
        ...
        if num_comps == 0:
            logger.debug(
                f"Label {supported_class_map[target_label]} has no component."
            )
        elif num_comps == 1:
            logger.debug(
                f"Label {supported_class_map[target_label]} has only one component."
            )
    ...
    def _get_largest_comp_idx_for_kidney(
        ...
        if largest_comp_idx == -1:
            if 1 not in seg_masks and 2 not in seg_masks:
                ...
                logger.debug(
                    f"No {supported_class_map[target_label]} candidate found below the adrenal gland."
                )
            logger.debug(
                f"Will use the global largest component as {supported_class_map[target_label]}."
            )
            comp_sizes = ndi.sum(
                seg_masks[target_label],
                labeled_comps,
...
#     return [i for i in data if lower_bound <= i[0] <= upper_bound]
...
def calculate_median(seg_data, labels: Union[List[int], int], median_points: List[np.ndarray]):
...
def cleanup(seg_masks: Dict[int, np.ndarray]) -> Dict[int, np.ndarray]:
    ...
    for label in target_labels:
        ...
        labeled_comp, num_comps = ndi.label(seg_masks[label])
        removed_count = 0
...
def split_left_right(seg_masks: Dict[int, np.ndarray]) -> Dict[int, np.ndarray]:
    ...
    for left_label, right_label in relevant_left_right_pairs.values():
        for current_label, opposite_label, expected_side in [
            ...
            labeled_comps, num_comps = ndi.label(seg_masks[current_label])
...
def suppress_non_largest_components(
    ...
    for label, mask in seg_masks.items():
        ...
        if label not in keep_fragmented_labels.values():
            labeled_components = cc3d.connected_components(mask, connectivity=6)
            component_sizes = np.bincount(labeled_components.ravel())
            component_sizes[0] = 0  # Ignore background
            largest_component_label = component_sizes.argmax()

            # Keep only the largest component
            seg_masks[label] = labeled_components == largest_component_label
...
def process_organs(data_input, debug=False):
    """
    ...
    """
    name2label_original = {name: label for label, name in supported_class_map.items()}

    source_dir, target_dir, new_class_map = data_input
    ...
    for f in seg_files:
        ...
        if name in name2label_original:  # supported labels
            seg_masks[name2label_original[name]] = nib.load(f).get_fdata().astype(bool)
        else:
            logger.debug(f"Class {name} not supported.")
            file_skipped.add(f)
    if len(seg_masks) == 0:
        logger.error(f"No valid segmentation files found in {source_dir.name}")
        return

    # standarize orientation
    logger.debug("Standardizing orientation")
    random_seg = nib.load(seg_files[0])
    affine = random_seg.affine
    header = random_seg.header
        ...
        try:
            ...
            if label == 6: # colon
                ...
                seg_masks = rectifier.reassign_components(
                    seg_masks,
                    label,
                    reassignment_candidates=list(large_organ_labels.values()),
                )
    ...
    inverse_transform = ornt_transform(target_ornt, orig_ornt)
    for label, data in seg_masks.items():
    ...
    for f in seg_files:
        ...
        corresponding_label = name2label_original.get(f.name.split(".", 1)[0], None)
        if corresponding_label is not None:
            ind_seg_mask = seg_masks[corresponding_label]
        else:
            ind_seg_mask = nib.load(f).get_fdata().astype(bool)
            for mask in seg_masks.values():
                ind_seg_mask = ind_seg_mask & ~mask.astype(bool)
    ...
    combined_seg = np.zeros(random_seg.shape, dtype=np.uint8)
    name2label_new = {name: label for label, name in new_class_map.items()}
    for f in file_skipped:
        mask = nib.load(f).get_fdata().astype(bool)
        combined_seg[mask.astype(bool)] = name2label_new[f.name.split(".", 1)[0]]
    for label, mask in sorted(seg_masks.items()):
        combined_seg[mask.astype(bool)] = name2label_new[supported_class_map[label]]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source_dir",
        default="data/JuMaMiniSC",
        type=str,
        help="Directory containing .nii.gz files to be postprocessed",
    )
    parser.add_argument(
        "--target_dir",
        default="data/JuMaMiniSC_processed",
        type=str,
        help="Directory to save the postprocessed files",
    )
    parser.add_argument(
        "--n_jobs",
        type=int,
        default=1,
        help="No of jobs for parallel processing the files",
    )
    parser.add_argument(
        "--class_map",
        type=str,
        default="class_map_abdomenatlas_1_1",
        help="Class map for the organs. Paste it to the top of the script.",
    )
    parser.add_argument(
        "--start_idx",
        type=int,
        default=0,
        help="Start index for selecting a subset of source directories"
    )
    parser.add_argument(
        "--end_idx",
        type=int,
        default=-1,
        help="End index (exclusive) for selecting a subset of source directories. Use -1 to process all"
    )
    ...
    n_jobs = args.n_jobs
    new_class_map = getattr(class_maps, args.class_map)

    source_sub_dirs = sorted([i for i in source_dir.iterdir() if i.name.startswith("BDMAP")], key=lambda x: x.name)
    if args.end_idx == -1:
        args.end_idx = len(source_sub_dirs)
    source_sub_dirs = source_sub_dirs[args.start_idx:args.end_idx]

    logger.info(f"Processing {len(source_sub_dirs)} source folders from index {args.start_idx} to {args.end_idx or 'end'}")

    logger.info(f"Start post-processing with {n_jobs} jobs")
    data_input = [
        (source_sub_dir, target_dir / source_sub_dir.name, new_class_map)
        for source_sub_dir in source_sub_dirs
    ]
    data_input = sorted(data_input, key=lambda x: x[0].name)
    if n_jobs > 1:
        with multiprocessing.Pool(n_jobs) as pool:
            pool.map(process_organs, data_input)
    else:
        for i in data_input:
            if "BDMAP_00027971" in i[0].name:
                logger.warning(
